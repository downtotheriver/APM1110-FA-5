...
The posterior probability of ***B*** given ***G*** or ***P(B\|G)*** is
the probability that ***B*** occurred given that ***G*** occurred. In
this problem, it means that if there will be a manager going for
upgrading of the computer hardware, what is the chance or probability
that that manager was also one of those managers who decided to buy a
new graphics package.

To solve this, we’ll use some data from (a), that is ***P(A)***,
***P(B)***, ***P(C)***, ***P(G\|A)***, ***P(G\|B)***, and ***P(G\|C)***
$$
...
The probability of a manager buying a new graphics package given that
he/she is upgrading is $0.875$ or $87.5$%.
...
![](SEC-1-FA5-Dacanay,-J_files/figure-gfm/unnamed-chunk-13-1.png)<!-- -->
...
![](SEC-1-FA5-Dacanay,-J_files/figure-gfm/unnamed-chunk-14-1.png)<!-- -->
...
|              | Percentage_of_Spyware | Probability_of_Detection |
|:------------:|:---------------------:|:------------------------:|
| **Internet** |          70           |           0.6            |
|  **Email**   |          30           |           0.8            |

Let ***I*** denote the spyware comes through the Internet while ***E***
the through email. Also, let ***D*** denote the anti-virus detector will
detect it while ***N*** the complementary of ***D***.

#### (a) What is the probability that this spyware infects the system?

Since the probability that the spyware will infect the system is what
being asked, it means that we need to get the probability that the
anti-virus detector will not detect the spyware. Only when the spyware
is not detected will it get to the system and infect it. Hence, we need
to get the total probability of the spyware not being detected. That is:
$$
...
The probability that the malicious spyware infects the system is $0.34$
or $34$%.

#### (b) If the spyware is detected, what is the probability that it came through the Internet?

To solve this, we’ll use some data from (a), that is ***P(I)***,
***P(D\|I)***, and ***P(N)***. Since we have solve for the total
probability of the spyware not being detected, we can get its
complementary to have the total probability of the spyware being
detected. $$
...
The probability of a manager buying a new graphics package given that
he/she is upgrading is $0.6364$ or $63.64$%.
